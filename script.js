...
const elements = {
    ...
    geminiKeyTestStatus: document.getElementById("gemini-key-test-status")
};
...
function setupEventListeners() {
    ...
    if (elements.btnTestGeminiKey) {
        elements.btnTestGeminiKey.addEventListener("click", async () => {
            ...
            if (!key) {
                alert("검증할 Gemini API Key를 입력해 주세요.");
...
const AIRCRAFT_IATA_MAP = {
    ...
    "E190": { name: "Embraer E190", id: "E190" },
    "E195": { name: "Embraer E195", id: "E195" },
    "E290": { name: "Embraer E190-E2", id: "E190-E2" },
    "E295": { name: "Embraer E195-E2", id: "E195-E2" },
...
function openGeminiKeyModal() {
    ...
    if (elements.geminiKeyTestStatus) {
        elements.geminiKeyTestStatus.className = "lookup-status-msg hidden";
        elements.geminiKeyTestStatus.textContent = "";
    }
...
/**
 * Gemini API Key 검증 테스트 함수
 */
async function testGeminiApiKey(apiKey) {
    if (!elements.geminiKeyTestStatus) return;

    elements.geminiKeyTestStatus.className = "lookup-status-msg loading";
    elements.geminiKeyTestStatus.innerHTML = `<span class="spinner"></span> Gemini API Key 상태를 확인하는 중입니다...`;
    if (elements.btnTestGeminiKey) elements.btnTestGeminiKey.disabled = true;
    ...
    try {
        ...
        if (res.ok) {
            ...
            elements.geminiKeyTestStatus.className = "lookup-status-msg success";
            elements.geminiKeyTestStatus.textContent = `✅ API Key 검증 완료! (사용 가능 모델: ${count}개)`;
        } else {
            let msg = `상태 코드 ${res.status}`;
            try {
        ...
        elements.geminiKeyTestStatus.className = "lookup-status-msg error";
        elements.geminiKeyTestStatus.textContent = `❌ 네트워크 오류: ${err.message || "연결 실패"}`;
    } finally {
        if (elements.btnTestGeminiKey) elements.btnTestGeminiKey.disabled = false;
    }
}
...
async function handleTicketFileSelected(event) {
    ...
    elements.lookupStatus.innerHTML = `<span class="spinner"></span> Gemini AI가 티켓 이미지에서 비행 정보(공항, 편명, 날짜, 좌석, 기종 등)를 분석하는 중입니다...`;
    ...
    try {
        ...
        if (parsedResult) {
            ...
            // 결과 자동 입력 (Auto-populate) 및 하이라이트 대상 수집
            if (parsedResult.departureAirport) {
                const depVal = parsedResult.departureAirport.trim().toUpperCase();
                elements.flightDepInput.value = depVal;
                const matchedDep = getAirportName(depVal);
                elements.flightDepMatch.textContent = matchedDep ? `출발지: ${matchedDep}` : "";
                filledFieldIds.push("flight-dep");
            }
            if (parsedResult.arrivalAirport) {
                const arrVal = parsedResult.arrivalAirport.trim().toUpperCase();
                elements.flightArrInput.value = arrVal;
                const matchedArr = getAirportName(arrVal);
                elements.flightArrMatch.textContent = matchedArr ? `도착지: ${matchedArr}` : "";
                filledFieldIds.push("flight-arr");
            }
            if (parsedResult.flightNumber) {
                const flightNumEl = document.getElementById("flight-number");
                if (flightNumEl) {
                    flightNumEl.value = parsedResult.flightNumber.trim().toUpperCase();
                    filledFieldIds.push("flight-number");
                }
            }
            if (parsedResult.date) {
                const flightDateEl = document.getElementById("flight-date");
                if (flightDateEl) {
                    flightDateEl.value = parsedResult.date.trim();
                    filledFieldIds.push("flight-date");
                }
            }
            if (parsedResult.airline) {
                const airlineEl = document.getElementById("flight-airline");
                if (airlineEl) {
                    airlineEl.value = parsedResult.airline.trim();
                    filledFieldIds.push("flight-airline");
                }
            }
            if (parsedResult.seat) {
                const seatVal = parsedResult.seat.trim().toUpperCase();
                const seatEl = document.getElementById("flight-seat");
                if (seatEl) {
                    seatEl.value = seatVal;
                    filledFieldIds.push("flight-seat");
                }
                const guessedPos = guessSeatPosition(seatVal);
                if (guessedPos && elements.seatPositionSelect) {
                    elements.seatPositionSelect.value = guessedPos;
                    filledFieldIds.push("flight-seat-position");
                }
            }
            if (parsedResult.aircraftTypeName) {
                const typeNameEl = document.getElementById("flight-typename");
                if (typeNameEl) {
                    typeNameEl.value = parsedResult.aircraftTypeName.trim();
                    filledFieldIds.push("flight-typename");
                    if (typeof mapAircraftTypeToId === "function") {
                        mapAircraftTypeToId(parsedResult.aircraftTypeName.trim());
                    }
                }
            }
            if (parsedResult.seatClass && elements.seatClassSelect) {
                const sc = parsedResult.seatClass.toLowerCase();
                if (["economy", "premium", "business", "first"].includes(sc)) {
                    elements.seatClassSelect.value = sc;
                    filledFieldIds.push("flight-seat-class");
                }
            }
            if (parsedResult.registration) {
                const regEl = document.getElementById("flight-registration");
                if (regEl) {
                    regEl.value = parsedResult.registration.trim().toUpperCase();
                    filledFieldIds.push("flight-registration");
                }
            }
            if (parsedResult.memo) {
                const memoEl = document.getElementById("flight-memo");
                if (memoEl) {
                    memoEl.value = parsedResult.memo.trim();
                    filledFieldIds.push("flight-memo");
                }
            }

            // 하이라이트 애니메이션 실행
            highlightAiFields(filledFieldIds);

            elements.lookupStatus.className = "lookup-status-msg success";
            elements.lookupStatus.textContent = "✨ Gemini AI가 티켓 사진 분석을 완료하여 비행 정보를 자동으로 채웠습니다!";
        } else {
...
/**
 * 자동 입력된 필드 시각 하이라이트 애니메이션 적용 함수
 */
function highlightAiFields(fieldIds) {
    fieldIds.forEach(id => {
        const el = document.getElementById(id);
        if (el) {
            el.classList.remove("ai-highlighted");
            void el.offsetWidth;
            el.classList.add("ai-highlighted");
            setTimeout(() => {
                el.classList.remove("ai-highlighted");
            }, 3600);
        }
...
/**
 * Gemini Vision REST API 호출 함수 (ListModels 자동 감지 및 후보 폴백 지원)
 */
        ...
        if (listRes.ok) {
            ...
            if (listData.models && listData.models.length > 0) {
                ...
                if (supported.length > 0) {
                    const flashModels = supported.filter(m => m.name.toLowerCase().includes("flash")).map(m => m.name);
                    const proModels = supported.filter(m => m.name.toLowerCase().includes("pro")).map(m => m.name);
                    const otherModels = supported.filter(m => !m.name.toLowerCase().includes("flash") && !m.name.toLowerCase().includes("pro")).map(m => m.name);

                    selectedModels = [...new Set([...flashModels, ...proModels, ...otherModels, ...candidateModels])];
                }
            }
        }
    } catch (e) {
        console.warn("Model list fetch failed, using default candidate fallback list", e);
    }

    const promptText = `Analyze this airline ticket, mobile boarding pass, or e-ticket receipt image carefully.
Extract as many flight details as possible and return ONLY valid JSON matching this schema:
{
  "departureAirport": "3-letter IATA airport code of departure (e.g. GMP, ICN, CJU, NRT)",
  "arrivalAirport": "3-letter IATA airport code of arrival (e.g. CJU, KIX, PUS)",
  "flightNumber": "Airline flight number (e.g. KE1101, OZ8902)",
  "date": "Flight date in YYYY-MM-DD format",
  "airline": "Airline name in Korean if available (e.g. 대한항공, 아시아나항공, 제주항공) or English",
  "seat": "Seat number (e.g. 42A, 12C)",
  "aircraftTypeName": "Aircraft type name if visible (e.g. Airbus A321neo, Boeing 777-300ER, A330-300)",
  "seatClass": "Seat class if visible: 'economy', 'premium', 'business', or 'first'",
  "registration": "Aircraft registration number if visible (e.g. HL8314)",
  "memo": "Any extra helpful note extracted from ticket (e.g. Boarding Time 14:20, Gate 15)"
}
If any field is not visible on the ticket, set its value to null.
Do not include any explanation or markdown codeblocks; output pure JSON only.`;
    ...
    for (const rawModelName of selectedModels) {
        const modelEndpoint = rawModelName.startsWith("models/") ? rawModelName : `models/${rawModelName}`;
    ...
    throw lastError || new Error("Gemini AI 모델 호출에 실패했습니다.");
}
