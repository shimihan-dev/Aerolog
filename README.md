...
1. **보딩패스 스타일 UI (Card-based Layout)**
   - 항공권 보딩패스(탑승권) 디자인을 본뜬 직관적이고 미려한 레이아웃을 통해 비행 정보를 한눈에 파악할 수 있습니다.
2. **실시간 비행 통계 (Summary Dashboard)**
   - 등록된 전체 비행 횟수, 고유 항공사 수, 고유 기종 수 등의 통계 지표를 실시간으로 합산하여 보여줍니다.
3. **실시간 검색 기능 (Search)**
   - 항공사, 편명, 기종명, 등록부호, 출발/도착 공항 코드(IATA) 등 다양한 키워드로 비행 기록을 즉시 필터링합니다.
4. **기록 추가 및 삭제 (CRUD)**
   - 전용 모달 폼을 통해 간편하게 기록을 추가하고, 확인 컨펌창을 통해 불필요한 비행을 쉽게 삭제할 수 있습니다.
5. **로컬스토리지 연동 (LocalStorage Persistence)**
   - 브라우저 데이터베이스인 LocalStorage를 사용하여 웹페이지를 새로고침하거나 브라우저를 껐다 켜도 데이터가 안전하게 유지됩니다.
6. **AeroType 연동 기종 표시 배지**
   - AeroType의 데이터 ID 스키마와 일치하는 기종 코드가 들어올 경우, 자동으로 'AeroType 연동 기종' 배지를 카드에 표시합니다.
...
- **배포 예고**: GitHub Pages를 통한 정적 배포 지원 예정

---

## 📊 데이터 구조 (Data Schema)

비행 기록은 `data/sampleFlights.js` 및 LocalStorage에서 다음과 같은 JSON 형태의 객체 구조로 영속화됩니다:
